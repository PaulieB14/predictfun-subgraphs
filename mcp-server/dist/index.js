...
async function predictApiFetch(path, params = {}) {
    if (!PREDICT_API_KEY)
    ...
    try {
        ...
        if (!res.ok)
            return { ok: false, data: null, rateLimitRemaining };
...
const marketIdCache = new Map(); // conditionId → numeric API id
        ...
        do {
            ...
            for (const m of data.data || []) {
                if (m.conditionId) {
                    ...
                    if (m.title || m.question)
                        marketNameCache.set(key, m.title || m.question);
                    if (m.id)
                        marketIdCache.set(key, m.id);
                }
            }
...
function contractLabel(addr) {
    ...
    if (info)
        return { is_contract: true, contract_name: info.name, contract_role: info.role };
...
server.tool("get_top_markets", "Get the top prediction markets ranked by volume, open interest, or trade count", {
    ...
    if (rank_by === "open_interest") {
        const data = await query(ENDPOINTS.positions, `{ conditions(first: ${limit}, orderBy: openInterest, orderDirection: desc, where: { openInterest_gt: "0" }) { id openInterest splitCount mergeCount resolved resolvedAt source outcomeSlotCount } }`);
        ...
        data.conditions.forEach((c, i) => {
            ...
            lines.push(`| ${i + 1} | ${c.id} | ${marketLabel(c.id, names)} | ${fmtUsd(c.openInterest)} | ${status} | ${c.splitCount} | ${c.mergeCount} |`);
        });
    }
    else {
        ...
        (data?.markets || []).forEach((m, i) => {
            lines.push(`| ${i + 1} | ${m.id} | ${marketLabel(m.id, names)} | ${fmtUsd(m.volume)} | ${parseInt(m.tradeCount).toLocaleString()} | ${fmtUsd(m.fees)} | ${m.exchange} |`);
        });
    }
    return { content: [{ type: "text", text: lines.join("\n") }] };
});
...
server.tool("get_market_details", "Get full details for a specific market/condition: volume, OI, resolution status, recent trades, top holders", {
    ...
    const [posData, obData, yieldData] = await Promise.all([
        ...
        query(ENDPOINTS.yield, `{ oracleRequests(first: 1, where: { requester: "${id}" }) { id settled settledAt } tokenMappings(first: 1) { underlying vToken enabled totalDeposited } }`),
    ]);
    ...
    if (posData.userPositions.length > 0) {
        const holderAddrs = posData.userPositions.map((p) => p.user.id);
...
// ─── Tool: Live Orderbook ────────────────────────────────────────────────────
server.tool("get_market_orderbook", "Get the live order book for a Predict.fun market — current bids and asks with prices and sizes. Requires PREDICT_API_KEY. Provide market_id (integer from Predict.fun API) or condition_id. Use this to see the current state of the market, not historical trades.", {
    condition_id: z
        .string()
        .optional()
    ...
    const allBids = ob.bids ?? [];
    const allAsks = ob.asks ?? [];
    const condKey = condition_id?.toLowerCase() ?? "";
    const names = condKey ? await resolveMarketNames([condKey]) : new Map();
    const title = condKey ? names.get(condKey) : undefined;
    const lines = [`# Live Orderbook: ${title || condition_id || `Market ${numericId}`}\n`];
    lines.push(`*Market ID: ${numericId}*`);
    if (rateLimitRemaining !== null)
        lines.push(`*Rate limit: ${rateLimitRemaining} remaining*`);
    lines.push("");
        ...
        if (posAcct) {
            ...
            if (pnlUnreliable) {
                ...
                try {
                    ...
                    const takerBuyVol = takerEvents.filter((f) => f.side === "BUY").reduce((s, f) => s + parseFloat(f.takerAmountFilled), 0);
                    const takerSellVol = takerEvents.filter((f) => f.side === "SELL").reduce((s, f) => s + parseFloat(f.takerAmountFilled), 0);
                    // As maker: BUY side means taker bought, maker sold tokens → USDC in for maker; SELL side = USDC out for maker
                    const makerBuyVol = makerEvents.filter((f) => f.side === "BUY").reduce((s, f) => s + parseFloat(f.makerAmountFilled), 0);
                    const makerSellVol = makerEvents.filter((f) => f.side === "SELL").reduce((s, f) => s + parseFloat(f.makerAmountFilled), 0);
                    const totalFeesSampled = [...takerEvents, ...makerEvents].reduce((s, f) => s + parseFloat(f.fee || "0"), 0);
                    ...
                    lines.push(`- As Taker — BUY fills (USDC out): ${fmtUsd(takerBuyVol.toString())} (${takerEvents.filter((f) => f.side === "BUY").length} fills)`);
                    lines.push(`- As Taker — SELL fills (USDC in): ${fmtUsd(takerSellVol.toString())} (${takerEvents.filter((f) => f.side === "SELL").length} fills)`);
                    lines.push(`- As Maker — BUY fills (USDC in): ${fmtUsd(makerBuyVol.toString())} (${makerEvents.filter((f) => f.side === "BUY").length} fills)`);
                    lines.push(`- As Maker — SELL fills (USDC out): ${fmtUsd(makerSellVol.toString())} (${makerEvents.filter((f) => f.side === "SELL").length} fills)`);
    ...
    switch (event_type) {
        ...
        case "splits": {
            ...
            data.splitEvents.forEach((e) => {
                lines.push(`| ${fmtDate(e.timestamp)} | ${e.stakeholder} | ${fmtUsd(e.amount)} | ${e.condition.id} | ${marketLabel(e.condition.id, splitNames)} |`);
            });
            break;
        }
        case "merges": {
            ...
            data.mergeEvents.forEach((e) => {
                lines.push(`| ${fmtDate(e.timestamp)} | ${e.stakeholder} | ${fmtUsd(e.amount)} | ${e.condition.id} | ${marketLabel(e.condition.id, mergeNames)} |`);
            });
            break;
        }
        case "redemptions": {
            ...
            data.redemptionEvents.forEach((e) => {
                const winner = e.condition.payoutNumerators
                    ? e.condition.payoutNumerators.indexOf("1") === 0
                        ...
                        : "No"
                    : "N/A";
                lines.push(`| ${fmtDate(e.timestamp)} | ${e.redeemer} | ${fmtUsd(e.payout)} | ${e.condition.id} | ${marketLabel(e.condition.id, redeemNames)} | ${winner} |`);
            });
            break;
        }
        case "yield_claims": {
            ...
            data.yieldClaimEvents.forEach((e) => {
                lines.push(`| ${fmtDate(e.timestamp)} | ${fmtUsd(e.underlyingAmount)} | ${e.vTokenAmount} | ${e.transactionHash} |`);
            });
            break;
        }
        case "all": {
            ...
            const unified = [];
            (tradesData?.orderFilledEvents || []).forEach((e) => {
            ...
            (splitsData?.splitEvents || []).forEach((e) => {
            ...
            (mergesData?.mergeEvents || []).forEach((e) => {
            ...
            (redemptionsData?.redemptionEvents || []).forEach((e) => {
            ...
            (yieldData?.yieldClaimEvents || []).forEach((e) => {
...
server.tool("get_yield_overview", "Get Venus Protocol yield stats: deposits, redemptions, net balance, yield claims, and token mappings", {}, async () => {
    ...
    if (data.yieldClaimEvents.length > 0) {
        ...
        lines.push("|---|---|---|");
        data.yieldClaimEvents.forEach((e) => {
            lines.push(`| ${fmtDate(e.timestamp)} | ${fmtUsd(e.underlyingAmount)} | ${e.transactionHash} |`);
        });
    }
    return { content: [{ type: "text", text: lines.join("\n") }] };
});
...
server.tool("get_whale_positions", "Find the largest position holders across all Predict.fun markets", {
    ...
    const obMap = new Map((obData?.accounts || []).map((a) => [a.id.toLowerCase(), a]));
    const yieldMap = new Map((yieldData?.yieldAccounts || []).map((a) => [a.id.toLowerCase(), a]));
    ...
    sliced.forEach((p, i) => {
        ...
        const ob = obMap.get(p.user.id.toLowerCase());
        const yld = yieldMap.get(p.user.id.toLowerCase());
        ...
        lines.push(`| ${i + 1} | ${p.user.id} | ${typeTag} | ${fmtUsd(p.netQuantity)} | ${fmtUsd(p.user.totalSplitVolume)} | ${ob ? fmtUsd(ob.totalVolume) : "—"} | ${yieldRew} | ${marketLabel(p.condition.id, whaleNames)} | ${fmtUsd(p.condition.openInterest)} | ${pctOi}% | ${flag} |`);
    });
    return { content: [{ type: "text", text: lines.join("\n") }] };
});
    ...
    if (rank_by === "payouts") {
        ...
        sliced.forEach((a, i) => {
            ...
            const pnl = parseFloat(a.totalPayouts) +
                ...
                splitVol;
            const ob = obMap.get(a.id.toLowerCase());
            const yld = yieldMap.get(a.id.toLowerCase());
            const obVol = ob ? parseFloat(ob.totalVolume) : 0;
            ...
            lines.push(`| ${i + 1} | ${a.id} | ${typeTag} | ${fmtUsd(a.totalPayouts)} | ${fmtUsd(a.totalSplitVolume)} | ${fmtUsd(a.totalMergeVolume)} | ${ob ? fmtUsd(ob.totalVolume) : "—"} | ${obFees} | ${yieldRew} | ${a.redeemCount} | ${pnlDisplay} |`);
        });
    }
    else {
        ...
        const posMap2 = new Map((posData2?.accounts || []).map((a) => [a.id.toLowerCase(), a]));
        const yieldMap2 = new Map((yieldData2?.yieldAccounts || []).map((a) => [a.id.toLowerCase(), a]));
        const addrTypes = await classifyAddresses(ids);
        ...
        sliced.forEach((a, i) => {
            const typeTag = addrTypeLabel(addrTypes.get(a.id.toLowerCase()));
            const pos2 = posMap2.get(a.id.toLowerCase());
            const yld2 = yieldMap2.get(a.id.toLowerCase());
            const payouts = pos2 ? fmtUsd(pos2.totalPayouts) : "—";
            const yieldRew = yld2 ? fmtUsd(yld2.totalRewardsClaimed) : "—";
            lines.push(`| ${i + 1} | ${a.id} | ${typeTag} | ${fmtUsd(a.totalVolume)} | ${parseInt(a.totalTrades).toLocaleString()} | ${fmtUsd(a.totalFees)} | ${parseInt(a.makerTrades).toLocaleString()} | ${parseInt(a.takerTrades).toLocaleString()} | ${payouts} | ${yieldRew} |`);
        });
    }
    return { content: [{ type: "text", text: lines.join("\n") }] };
});
...
server.tool("get_resolved_markets", "Get recently resolved markets with their outcomes and payout info", {
    ...
    data.conditions.forEach((c, i) => {
        ...
        lines.push(`| ${i + 1} | ${c.id} | ${marketLabel(c.id, resolvedNames)} | ${winner} | ${fmtUsd(c.openInterest)} | ${c.splitCount} | ${fmtDate(c.resolvedAt)} |`);
    });
    return { content: [{ type: "text", text: lines.join("\n") }] };
});
// ─── Tool: Custom Query ──────────────────────────────────────────────────────
...
server.tool("find_trader_persona", "Classify a trader into behavioral archetypes: whale_accumulator, yield_farmer, arbitrageur, early_mover, or resolution_sniper. Returns structured JSON with matched personas and supporting metrics.", {
    ...
    for (const p of positions) {
        ...
        if (parseFloat(p.totalSplit) > 0) {
            ...
            try {
                const obFillData = await query(ENDPOINTS.orderbook, `{ orderFilledEvents(first: 1, orderBy: timestamp, orderDirection: asc, where: { taker: "${addr}", market: "${p.condition.id}" }) { timestamp } }`);
    ...
    for (const p of resolvedPositions.slice(0, 5)) {
        ...
        try {
            if (!isObOnly) {
                ...
                const obFillData = await query(ENDPOINTS.orderbook, `{ orderFilledEvents(first: 5, orderBy: takerAmountFilled, orderDirection: desc, where: { taker: "${addr}", market: "${p.condition.id}", timestamp_gt: "${windowStart}", timestamp_lt: "${resolvedAt}" }) { takerAmountFilled timestamp } }`);
                if (obFillData?.orderFilledEvents?.length > 0) {
                    const totalLateVolume = obFillData.orderFilledEvents.reduce((sum, f) => sum + parseFloat(f.takerAmountFilled), 0);
...
server.tool("tag_market_structure", "Classify a market by structural features: resolution latency, liquidity profile, oracle type, and tail-risk indicators. Returns structured JSON.", {
    ...
    if (cond) {
        ...
        if (oracleInfo.is_contract) {
            ...
            if (contractName === "NegRiskAdapter")
                oracleTag = "neg_risk_adapter";
            else if (contractName === "CTFOracle")
                oracleTag = "ctf_oracle";
            else if (contractName === "YieldOracle")
                oracleTag = "yield_oracle";
        }
